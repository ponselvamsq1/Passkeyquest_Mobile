...
public class PlayerStateManager : MonoBehaviour
{
    ...
    [Header("UI Tips")]
    [SerializeField] private GameObject uiTipsPanel;
    public TextMeshProUGUI tipsHeadText,tipsContentTextx;
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        //Write PS
        if (collision.collider.CompareTag("Key1"))
        {
            collision.gameObject.SetActive(false);
            collision.gameObject.GetComponent<AudioSource>().Play();
            PlayerPrefs.SetInt("keyID1", 1);
            Increasekeyvalue();
            
        }

        if (collision.collider.CompareTag("Key2"))
        {
            collision.gameObject.SetActive(false);
            collision.gameObject.GetComponent<AudioSource>().Play();
            PlayerPrefs.SetInt("keyID2", 1);
            Increasekeyvalue();
        }

        if (collision.collider.CompareTag("Key3"))
        {
            collision.gameObject.SetActive(false);
            collision.gameObject.GetComponent<AudioSource>().Play();
            PlayerPrefs.SetInt("keyID3", 1);
            Increasekeyvalue();
        }

        if (collision.collider.CompareTag("Key4"))
        {
            collision.gameObject.SetActive(false);
            collision.gameObject.GetComponent<AudioSource>().Play();
            PlayerPrefs.SetInt("keyID4", 1);
            Increasekeyvalue();
        }
        if (collision.collider.CompareTag("Key5"))
        {
            collision.gameObject.SetActive(false);
            collision.gameObject.GetComponent<AudioSource>().Play();
            PlayerPrefs.SetInt("keyID5", 1);
            Increasekeyvalue();
        }
    }
    ...
    void Increasekeyvalue()
    {
        ...
        {
            case 1:
                tipsHeadText.text =  GameManager.Instance.Keycount +". Tips to create a strong password";
                tipsContentTextx.text = "\n• A strong password is the first line of defense for protecting personal information. It needs to be robust to provide a solid security barrier. \n \n• Longer passwords are significantly harder to crack through brute-force attacks.\n \n• Avoid using easily predictable data like your name, birthdate, phone number, or common dictionary words. \n \n •Create passwords in a complex and unpredictable sequence, with a random combination of uppercase and lowercase letters, numbers, and a variety of symbols. \n \n• For example, cXmnZK65rf*&DaaD, incorporating the characteristics of length, complexity and uniqueness ";
                break;
            case 2:
                tipsHeadText.text =  GameManager.Instance.Keycount+". Introducing the Passphrase Approach";
                tipsContentTextx.text = "\n• Instead of using a complex string of random characters, create a memorable phrase known as a Passphrase. This method involves using a sequence of unrelated words to construct a strong password. \n \n• Choose 4 to 7 unrelated words that make it harder for someone to guess. \n \n• Adding more words to your passphrase increases its length and security significantly. For example,HorsePurpleHatRunBay is stronger than HorsePurpleHatRun.\n \n• To easily memorize your passphrase, use a phrase that you frequently say or one that relates to the website you are securing. Be sure to include a combination of uppercase and lowercase letters, as well as symbols.\n \n• Consider a poem that you remember well. By substituting a few letters with numbers and symbols, you can create a robust password.";
                break;
            case 3:
                tipsHeadText.text = GameManager.Instance.Keycount +". Enabling Multifactor Authentication (MFA)";
                tipsContentTextx.text = "\n• Always enable MFA whenever available for online accounts. This adds an extra layer of security to protect your information. \n \n• Multifactor Authentication (MFA) is a method to confirm your identity. It requires a second credential called a second  factor—to prove who you are. Eg., a code from an app or a fingerprint.\n \n• The second factor of authentication generally falls into one of three categories: Something you know (e.g., a personal identification number), something you have (e.g., a smartphone), or something you are (e.g., fingerprint recognition).\n \n• If someone attempts to sign in as you and knows your username and password, they will be blocked when prompted for the second factor. Without your smartphone, they cannot access the 6-digit code required for entry.";
                break;
            case 4:
                tipsHeadText.text =  GameManager.Instance.Keycount+". Understanding Common Password Attack Methods";
                tipsContentTextx.text = "\n• Be aware that attackers use various social engineering tactics like phishing, brute force, dictionary attacks, credential stuffing, and pretexting.\n \n• Criminals often try to trick you into revealing your password, as it's often easier to exploit human nature.\n \n• Always be suspicious of unexpected requests for sensitive information, even if they seem to come from someone you know or a trusted company. \n \n• Never share your password in response to an email or phone call, even if it seems legitimate. \n \n• To access important services like banking, always go directly to their official website using your own saved bookmark or by typing the correct web address yourself.";
                break;
            case 5:
                tipsHeadText.text =  GameManager.Instance.Keycount+". Utilize a Password Manager";
                BuffsManager.Instance.Lasttip();
                tipsContentTextx.text = "\n• Don't reuse passwords; a compromised account can affect all others using the same password.\n• Create strong passwords (at least 8 characters) using a combination of letters, numbers, and symbols.\n• If you suspect a security breach, change your password immediately. Otherwise, use a password manager for strong, unique passwords.";
                break;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        else if (collision.CompareTag(GameConstants.PORTAL_TAG))
        {
            ...
            if (GameManager.Instance.Keycount == 5)
            {
                ...
                BuffsManager.Instance.GameWin();
            }
            else
            {
                BuffsManager.Instance.GameLoss();
            }
    ...
    {
       // if (_hasWinGame && !GameManager.Instance.iscontinue) return;
